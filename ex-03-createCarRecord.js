...
function createCarRecord(makeStr,modelStr,yearNum,plateStr){
  var carRecord = {};
  if (makeStr === undefined || modelStr === undefined || yearNum === undefined || plateStr === undefined) {
    return "new record must have make, model, year, and license values";
  } else if (typeof makeStr !== "string" || typeof modelStr !== "string" || typeof plateStr !== "string") {
    return "1st, 2nd, and 4th arguments must be String";
  } else if (typeof yearNum !== "number") {
    return "3rd argument must be a Number";
  } else {
    carRecord.make = makeStr;
    carRecord.model = modelStr;
    carRecord.year = yearNum;
    carRecord.license = plateStr;
  }
  return carRecord;
}
